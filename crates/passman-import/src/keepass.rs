...
//!
//! * **Group structure.** kdbx is a tree; passman's collections are flat. The
...
//!   decides whether passman masks them. Guessing from the name instead would
...
/// Map one kdbx entry onto a passman item.
pub fn map_entry(entry: KdbxEntry<'_>) -> Item {
    ...
    item.attributes
        .insert("passman:source".into(), "keepass".into());
