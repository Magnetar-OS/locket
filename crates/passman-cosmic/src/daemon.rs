...
/// Bus names to look for, in order. The daemon defaults to the passman name so
/// it does not displace gnome-keyring, but takes the freedesktop name with
/// `--replace-keyring`.
const BUS_NAMES: &[&str] = &[
    passman_secret::WELL_KNOWN_NAME,
    passman_secret::DEV_NAME,
];

const MANAGER_PATH: &str = "/org/passman/Manager";

#[zbus::proxy(interface = "org.passman.Manager1", assume_defaults = false)]
trait Manager {
...
async fn connect() -> Option<(zbus::Connection, ManagerProxy<'static>)> {
    ...
    for name in BUS_NAMES {
        ...
        if let Some(proxy) = proxy
            && proxy.locked().await.is_ok()
        {
            tracing::info!("connected to passmand on {name}");
...
pub fn subscription() -> Subscription<DaemonEvent> {
    Subscription::run(|| {
        stream::channel(8, |mut tx: mpsc::Sender<DaemonEvent>| async move {
            ...
            loop {
                let Some((_connection, proxy)) = connect().await else {
