...
//! `gnome-keyring` — and writes it into a locket vault. This is the migration
...
/// Map a foreign item onto a locket item.
///
...
pub fn map_item(
    ...
    for key in ["username", "user", "UserName", "login"] {
        if let Some(v) = item.attributes.get(key) {
            let v = v.clone();
            item.set_field(locket_core::Field::text(
                locket_core::model::field_names::USERNAME,
                v,
            ));
            break;
        }
    ...
    for key in ["url", "uri", "server", "host"] {
        if let Some(v) = item.attributes.get(key) {
            let v = v.clone();
            item.set_field(locket_core::Field::new(
                locket_core::model::field_names::URL,
...
mod tests {
    use super::*;
    use locket_core::{crypto::KdfParams, model::field_names};
    ...
    fn replace_overwrites_a_matching_item_in_place() {
        use locket_core::model::Collection;
        ...
        let mut corrupted = locket_core::model::Item::new(
            locket_core::model::ItemKind::Application,
