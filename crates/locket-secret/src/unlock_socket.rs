...
//!
//! Accepts a passphrase from a local client — in practice `pam_locket.so` at
//! login — and unlocks the vault with it. See [`locket_ipc`] for why the
...
async fn rekey(state: &SharedState, vault_path: &Path, old: &str, new: &str) -> bool {
    ...
    let result = tokio::task::spawn_blocking(move || -> locket_core::Result<()> {
        let mut vault = Vault::open(&path, &old)?;
        vault.change_passphrase(&new, locket_core::crypto::KdfParams::default())?;
        vault.save()
    })
    .await;
...
async fn handle(
    ...
    let mut framed = zeroize::Zeroizing::new(Vec::new());
    {
        ...
        if body_len > 2 * locket_ipc::MAX_PASSPHRASE_LEN + 9 {
            // Refuse rather than allocate on a bad client's say-so.
            let _ = stream.write_all(&[locket_ipc::REPLY_REFUSED]).await;
            return Ok(());
        }
    ...
    let request = match locket_ipc::read_request(&mut framed.as_slice()) {
        Ok(r) => r,
        Err(e) => {
