...
enum Command {
    ...
    /// configuration from locket instead.
    ImportEnv {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    } = &args.command
    {
        ...
        let found = locket_import::cloud::scan(&home);
        if found.is_empty() {
    ...
    } = &args.command
    {
        ...
        for f in &files {
            ...
            let s = parsed
                .iter()
                .filter(|v| locket_import::dotenv::is_secret(&v.key, &v.value))
                .count();
            vars += parsed.len();
            secrets += s;
    ...
    match args.command {
        ...
        Command::Edit {
            ...
            for pair in &set {
                ...
                let kind = if locket_import::dotenv::is_secret(name, value) {
                    locket_core::model::FieldKind::Secret
                } else {
                    locket_core::model::FieldKind::Text
                };
        ...
        Command::Import {
            ...
            let summary = tokio::runtime::Runtime::new()?.block_on(
                locket_secret::import::import_from(&mut vault, &from, into.as_deref(), replace),
            )?;
        ...
        Command::ImportSsh { dir, into } => {
            let dir = match dir.or_else(locket_import::ssh::default_dir) {
                Some(d) => d,
...
impl From<EnvGrouping> for locket_import::dotenv::Grouping {
