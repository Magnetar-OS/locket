...
//! passman vault.
//!
//! This is the crate that lets passman stand in for `gnome-keyring-daemon`.
...
/// A parallel name, so passman can be exercised against real clients without
/// evicting the running keyring first.
pub const DEV_NAME: &str = "org.passman.secrets";
...
/// as a child node during introspection; passman matches that behaviour.
