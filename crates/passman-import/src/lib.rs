...
//!
//! Each importer maps a foreign format onto passman's item model and hands the
...
//! `passman-secret` instead, because it needs D-Bus rather than a file format.
...
mod tests {
    use super::*;
    use passman_core::{crypto::KdfParams, model::{Item, ItemKind}};
    ...
    fn duplicate_detection_needs_an_exact_attribute_match() {
        ...
        let item = Item::new(ItemKind::Login, "X")
            ...
            .with_attribute("passman:source", "pass");
